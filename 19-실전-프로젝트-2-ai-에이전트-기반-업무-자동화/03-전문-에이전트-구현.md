...
이 섹션에서는 [19.1 에이전트 아키텍처 설계](./01-에이전트-아키텍처-설계.md)에서 그린 청사진과 [19.2 외부 서비스 도구 구현](./02-외부-서비스-도구-구현.md)에서 만든 도구들을 결합하여, 실제로 동작하는 **전문 에이전트(Specialized Agent)** 4종을 구현합니다. 각 에이전트는 LangGraph의 노드로 동작하며, 자신만의 도구 세트와 시스템 프롬프트를 가집니다.
...
`create_react_agent`가 반환하는 것은 `CompiledGraph` 객체인데요, 이것은 그 자체로 `Runnable`이면서 동시에 상위 `StateGraph`의 노드로 삽입할 수 있습니다. 앞서 [Ch5 LCEL 마스터](05-lcellangchain-expression-language-마스터/01-lcel-기초와-파이프-연산자.md)에서 배운 `Runnable` 인터페이스 덕분에 `invoke`, `stream`, `batch` 모두 지원됩니다.
...
전문 에이전트들이 자율적으로 작동하는 건 좋지만, 이메일 전송이나 일정 생성처럼 **되돌리기 어려운 작업**을 에이전트가 마음대로 실행하면 위험하겠죠? 다음 섹션 **[19.4 Human-in-the-Loop 안전 메커니즘](./04-워크플로우-오케스트레이션.md)**에서는 LangGraph의 `interrupt` 기능을 활용하여, 위험한 도구 호출 전에 사용자 승인을 요구하는 안전장치를 구현합니다. 이번 세션에서 일정 에이전트 프롬프트에 넣었던 "승인 필요" 표시가 실제 코드 레벨에서 어떻게 동작하는지 확인할 수 있습니다.
...
- [search_emails_tool](./02-외부-서비스-도구-구현.md) (prerequisite)
- [send_email_tool](./02-외부-서비스-도구-구현.md) (prerequisite)
- [calendar_event_tool](./02-외부-서비스-도구-구현.md) (prerequisite)
- [query_database_tool](./02-외부-서비스-도구-구현.md) (prerequisite)
