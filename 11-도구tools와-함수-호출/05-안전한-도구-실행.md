...
이 섹션에서는 LLM이 도구를 호출할 때 발생할 수 있는 보안 위험을 이해하고, 이를 방어하는 4가지 핵심 안전 계층을 구축합니다. [세션 11.4: 고급 도구 패턴](./04-고급-도구-패턴.md)에서 배운 ToolException과 재시도 패턴이 **"도구가 실패했을 때"**의 대응이었다면, 이번 세션은 **"도구가 위험한 일을 하려 할 때"**의 대응입니다.

**선수 지식**: 
- [세션 11.1](./01-도구-정의와-바인딩.md)의 도구 정의와 bind_tools
- [세션 11.3](./03-도구-호출-처리.md)의 도구 실행 자동화 루프(tool_loop)
- [세션 11.4](./04-고급-도구-패턴.md)의 ToolException, handle_tool_error, with_retry
...
LangGraph에서는 `interrupt` 함수를 사용해 더 세련된 HITL 워크플로우를 구현할 수 있습니다. 이 내용은 [챕터 13: LangGraph 기초](13-langgraph-기초/01-langgraph-소개와-핵심-개념.md)에서 본격적으로 다루지만, 핵심 아이디어를 미리 살펴봅시다.
...
다음 [챕터 12: 에이전트(Agent) 기초](12-에이전트agent-기초/01-에이전트-개념과-react-패턴.md)에서는 이 도구들을 **자율적으로 선택하고 조합하는 에이전트**를 구축합니다. 도구는 에이전트의 "손과 발"이고, 이번 챕터에서 배운 안전 장치들은 에이전트가 그 손발을 **책임감 있게** 사용하도록 하는 제어 장치가 됩니다. 이 둘이 만나면 진정한 의미의 **자율 AI 시스템**이 탄생하죠.
...
---
### 🔗 Related Sessions
- [tool](./01-도구-정의와-바인딩.md) (prerequisite)
...
- [toolexception](./04-고급-도구-패턴.md) (prerequisite)
- [handle_tool_error](./04-고급-도구-패턴.md) (prerequisite)
