...
**선수 지식**: [12.1 에이전트 개념과 ReAct 패턴](./01-에이전트-개념과-react-패턴.md)에서 배운 ReAct 루프 동작 원리, [12.2 create_react_agent로 에이전트 구축](./02-create-react-agent로-에이전트-구축.md)의 에이전트 생성법, [12.3 AgentExecutor 설정과 제어](./03-agentexecutor-설정과-제어.md)의 `max_iterations`와 `handle_parsing_errors` 파라미터, [12.4 에이전트 도구 설계](./04-에이전트-도구-설계.md)의 도구 설명 최적화 기법
...
앞서 [12.3 AgentExecutor 설정과 제어](./03-agentexecutor-설정과-제어.md)에서 `max_iterations`나 `handle_parsing_errors` 같은 안전장치를 배웠는데요, 이번 섹션에서는 그 안전장치가 **왜** 작동했는지, 에이전트가 **어떤 과정**을 거쳐 그 상태에 도달했는지를 추적하는 기술을 배웁니다.
...
[12.3 AgentExecutor 설정과 제어](./03-agentexecutor-설정과-제어.md)에서 `return_intermediate_steps=True`를 배웠는데, 이번에는 이 데이터를 **체계적으로 분석**하는 방법을 살펴봅니다.
...
- [agent_executor_parameters](./02-create-react-agent로-에이전트-구축.md) (prerequisite)
- [max_iterations_config](./03-agentexecutor-설정과-제어.md) (prerequisite)
- [handle_parsing_errors_patterns](./03-agentexecutor-설정과-제어.md) (prerequisite)
- [return_intermediate_steps_usage](./03-agentexecutor-설정과-제어.md) (prerequisite)
- [tool_description_best_practice](./04-에이전트-도구-설계.md) (prerequisite)
