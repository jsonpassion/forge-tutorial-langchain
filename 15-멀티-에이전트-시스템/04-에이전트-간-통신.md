...
이 섹션에서는 멀티 에이전트 시스템의 핵심 인프라인 **에이전트 간 통신(Inter-agent Communication)** 을 다룹니다. 앞서 [15.1: 멀티 에이전트 아키텍처 패턴](./01-멀티-에이전트-아키텍처-패턴.md)에서 감독자, 계층적, 스웜 패턴의 구조를 배웠고, [15.2: 감독자 기반 멀티 에이전트](./02-감독자-기반-멀티-에이전트.md)에서 `create_supervisor`로 워크플로우를 구성했으며, [15.3: 전문화된 에이전트 설계](./03-전문화된-에이전트-설계.md)에서 역할별 에이전트를 만들었습니다. 이제 이 에이전트들이 **어떻게 정보를 주고받는지**, 그 구체적인 메커니즘을 파헤쳐 보겠습니다.
...
[15.2: 감독자 기반 멀티 에이전트](./02-감독자-기반-멀티-에이전트.md)에서 `create_handoff_tool`을 사용해 봤는데요, 이번에는 그 **내부 동작 원리**와 **커스터마이징**을 살펴보겠습니다.
...
이번 세션에서 에이전트 간 통신의 메커니즘을 배웠다면, 다음 세션 **[15.5: 멀티 에이전트 디버깅과 최적화](./05-멀티-에이전트-최적화.md)** 에서는 이 통신이 **제대로 작동하는지 어떻게 확인하고, 문제가 생겼을 때 어떻게 디버깅하는지**를 다룹니다. LangSmith를 활용한 트레이싱, 에이전트 간 병목 탐지, 그리고 성능 최적화 전략까지 — 프로덕션 수준의 멀티 에이전트 시스템을 완성하는 마지막 퍼즐 조각을 맞춰보겠습니다.
...
- [handoff](./01-멀티-에이전트-아키텍처-패턴.md) (prerequisite)
- [worker_agent](./02-감독자-기반-멀티-에이전트.md) (prerequisite)
- [create_handoff_tool](./01-멀티-에이전트-아키텍처-패턴.md) (prerequisite)
