...
**선수 지식**: [Ch5: LCEL 마스터](ch05)에서 배운 파이프 연산자(`|`)와 Runnable 인터페이스, [Ch16: 콜백과 관찰 가능성](ch16)에서 다룬 스트리밍 개념
**학습 목표**:
...
> 📊 **그림 1**: LangServe 아키텍처 — 체인에서 API까지

```mermaid
flowchart LR
    A["LangChain 체인<br/>(Runnable)"] --> B["LangServe<br/>add_routes"]
    B --> C["FastAPI 앱"]
    C --> D["/invoke"]
    C --> E["/stream"]
    C --> F["/batch"]
    C --> G["/playground"]
    C --> H["/input_schema<br/>/output_schema"]
    D & E & F --> I["클라이언트<br/>(웹, 모바일, 서버)"]
```
...
> 📊 **그림 2**: 하나의 FastAPI 앱에 여러 체인 등록

```mermaid
graph TD
    APP["FastAPI 앱"] --> R1["/translate"]
    APP --> R2["/summarize"]
    APP --> R3["/chat"]
    R1 --> C1["번역 체인<br/>prompt - model - parser"]
    R2 --> C2["요약 체인<br/>prompt - model - parser"]
    R3 --> C3["ChatOpenAI<br/>모델 직접 호출"]
    R1 --> P1["/translate/playground"]
    R2 --> P2["/summarize/playground"]
    R3 --> P3["/chat/playground"]
```
...
sequenceDiagram
    ...
    participant R as Runnable 체인
    Note over C,R: invoke (동기 호출)
    C->>S: POST /invoke (input)
    S->>R: runnable.invoke(input)
    R-->>S: 결과
    S-->>C: JSON 응답
    Note over C,R: stream (스트리밍)
    C->>S: POST /stream (input)
    S->>R: runnable.stream(input)
    loop 토큰 단위
        R-->>S: chunk
        S-->>C: SSE event
    end
    Note over C,R: batch (배치)
    C->>S: POST /batch (inputs)
    S->>R: runnable.batch(inputs)
    R-->>S: 결과 목록
    S-->>C: JSON 응답
```
...
> 📊 **그림 4**: 입출력 스키마 자동 생성과 활용

```mermaid
flowchart TD
    A["Runnable 체인"] --> B["타입 정보 분석<br/>(Pydantic)"]
    B --> C["JSON Schema 생성"]
    C --> D["입력 검증<br/>잘못된 요청 자동 거부"]
    C --> E["Swagger UI<br/>/docs 자동 문서화"]
    C --> F["Playground<br/>폼 위젯 자동 구성"]
    C --> G["클라이언트 SDK<br/>타입 안전 호출"]
```
