...
**선수 지식**: [17.1 LangServe 기초](ch17/session1.md)에서 배운 `add_routes`와 기본 `RemoteRunnable` 개념, [17.2 FastAPI 통합과 서버 구성](ch17/session2.md)에서 다룬 다중 체인 배포, [17.3 인증과 보안](ch17/session3.md)에서 학습한 API 키/JWT 인증
...
> 📊 **그림 1**: RemoteRunnable의 핵심 — 원격 체인을 로컬처럼 사용

```mermaid
flowchart LR
    subgraph 클라이언트
        A["RemoteRunnable"] -->|invoke, stream, batch| B["httpx 클라이언트"]
    end
    B -->|HTTP POST| C["LangServe 서버"]
    subgraph 서버
        C --> D["체인 실행"]
        D --> E["LLM / 도구"]
    end
...
> 📊 **그림 2**: invoke vs ainvoke 동작 방식 비교

```mermaid
sequenceDiagram
    participant C as 클라이언트
    participant S as LangServe 서버
    Note over C,S: invoke (동기) — 블로킹
    C->>S: POST /invoke
    Note over C: 대기 중...
    S-->>C: 전체 응답 반환
    Note over C,S: ainvoke (비동기) — 논블로킹
    C->>S: POST /invoke
    Note over C: 다른 작업 수행 가능
    S-->>C: 전체 응답 반환
```
...
> 📊 **그림 4**: 로컬-원격 체인 합성 파이프라인

```mermaid
flowchart LR
    A["로컬<br/>프롬프트 템플릿"] -->|파이프 연산자| B["원격 서버 A<br/>번역 체인"]
    B -->|파이프 연산자| C["원격 서버 B<br/>요약 체인"]
    C --> D["최종 결과"]
    style A fill:#e1f5fe
    style B fill:#fff3e0
    style C fill:#fff3e0
...
stateDiagram-v2
    [*] --> 요청전송
    요청전송 --> 성공: 200 OK
    요청전송 --> 연결실패: ConnectError
    요청전송 --> 타임아웃: TimeoutException
    요청전송 --> 클라이언트에러: 4xx
    요청전송 --> 서버에러: 5xx
    연결실패 --> 재시도판단
    타임아웃 --> 재시도판단
    재시도판단 --> 요청전송: 시도 횟수 미초과
    재시도판단 --> 최종실패: 3회 초과
    클라이언트에러 --> 입력수정: 422 형식 오류
    클라이언트에러 --> 인증확인: 401 인증 실패
    클라이언트에러 --> 대기후재시도: 429 한도 초과
    서버에러 --> 재시도판단
    성공 --> [*]
```
...
> 이 실습은 [17.1 LangServe 기초](ch17/session1.md)와 [17.3 인증과 보안](ch17/session3.md)에서 배포한 서버가 실행 중이라고 가정합니다.
...
이제 RemoteRunnable로 서버를 자유자재로 호출할 수 있게 되었습니다. 다음 섹션 **[17.5 배포 전략과 모니터링](ch17/session5.md)**에서는 Docker 컨테이너화, 클라우드 배포, 로드 밸런싱, 그리고 LangSmith를 활용한 프로덕션 모니터링까지 — LangServe 애플리케이션을 실제 서비스로 운영하는 전체 과정을 다룹니다.
...
- [cors_middleware](../17-langserve와-api-배포/02-fastapi-통합과-서버-구성.md) (prerequisite)
- [api_key_auth](../17-langserve와-api-배포/02-fastapi-통합과-서버-구성.md) (prerequisite)
