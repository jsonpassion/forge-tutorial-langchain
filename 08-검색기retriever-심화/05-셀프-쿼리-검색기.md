...
**선수 지식**: [8.1 검색기 기초](./01-검색기-기초.md)에서 배운 VectorStoreRetriever와 search_kwargs, [8.4 컨텍스트 압축](./04-컨텍스트-압축.md)에서 다룬 후처리 기반 검색 최적화 개념
...
[8.1 검색기 기초](./01-검색기-기초.md)에서 배운 VectorStoreRetriever는 세 번째만 처리할 수 있었고, [8.4 컨텍스트 압축](./04-컨텍스트-압축.md)의 방식으로는 일단 모든 문서를 검색한 뒤에야 불필요한 결과를 걸러낼 수 있었습니다. SelfQueryRetriever는 **검색 단계에서부터** 메타데이터 조건을 적용하여, 처음부터 조건에 맞는 문서만 검색 대상으로 삼습니다. 이것은 성능과 정확도 모두에서 큰 차이를 만듭니다.
...
> 🔥 **실무 팁**: 프로덕션에서는 `SelfQueryRetriever`와 [8.4 컨텍스트 압축](./04-컨텍스트-압축.md)의 `ContextualCompressionRetriever`를 **결합**하면 강력합니다. SelfQueryRetriever가 메타데이터 기반 전처리를, ContextualCompressionRetriever가 LLM 기반 후처리를 담당하는 2단계 파이프라인이죠:
> ```python
...
- [search_kwargs](./01-검색기-기초.md) (prerequisite)
- [contextualcompressionretriever](./04-컨텍스트-압축.md) (prerequisite)
- [llmchainfilter](./04-컨텍스트-압축.md) (prerequisite)
