...
> 📊 **그림 4**: 단일 쿼리 검색의 한계와 멀티쿼리 해결 전략

```mermaid
flowchart TD
    subgraph PROBLEM["단일 쿼리의 한계"]
        direction TB
        U1["사용자: '에러 처리'"] --> S1["검색 결과 2건"]
        P1["문서: '예외 핸들링'"] -.-x S1
    end
    subgraph SOLUTION["멀티쿼리 전략"]
        direction TB
        U2["사용자: '에러 처리'"] --> LLM["LLM 쿼리 확장"]
        LLM --> Q1["'에러 처리'"]
        LLM --> Q2["'예외 핸들링'"]
        LLM --> Q3["'오류 관리 방법'"]
        Q1 --> S2["검색 결과 7건"]
...
> 📊 **그림 1**: MultiQueryRetriever의 3단계 처리 흐름

```mermaid
flowchart LR
    Q["사용자 질문"] --> LLM["LLM<br/>쿼리 생성"]
    LLM --> Q1["변형 쿼리 1"]
    LLM --> Q2["변형 쿼리 2"]
    LLM --> Q3["변형 쿼리 3"]
    Q1 --> S1["검색 결과 A"]
...
> 📊 **그림 2**: MultiQueryRetriever vs RAG Fusion — 결과 결합 방식 비교

```mermaid
flowchart TD
    subgraph MQ["MultiQueryRetriever"]
        direction TB
        MR1["쿼리1 결과<br/>Doc A(1등), Doc B(2등)"] --> MU["합집합<br/>순위 무시"]
        MR2["쿼리2 결과<br/>Doc C(1등), Doc A(2등)"] --> MU
        MU --> MO["A, B, C<br/>순서 보장 없음"]
    end
    subgraph RF["RAG Fusion"]
        direction TB
        RR1["쿼리1 결과<br/>Doc A(1등), Doc B(2등)"] --> RRF["RRF 점수 계산<br/>순위 반영"]
        RR2["쿼리2 결과<br/>Doc C(1등), Doc A(2등)"] --> RRF
        RRF --> RO["A(최고점), C, B<br/>점수 기반 정렬"]
    end
```
...
> 📊 **그림 3**: LCEL RAG Fusion 체인의 전체 데이터 흐름

```mermaid
flowchart LR
    Q["사용자 질문"] --> GQ["generate_queries<br/>프롬프트+LLM+파서"]
    GQ --> MAP["retriever.map()<br/>병렬 검색"]
    MAP --> RRF["reciprocal_rank_fusion<br/>재순위화"]
    RRF --> FMT["format_docs<br/>텍스트 변환"]
    FMT --> CTX{"context + question"}
    Q --> CTX
    CTX --> ANS["answer_prompt<br/>+ LLM"]
    ANS --> OUT["최종 답변"]
```
