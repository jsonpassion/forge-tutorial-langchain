...
**선수 지식**: [8.1 검색기 기초](./01-검색기-기초.md)에서 배운 VectorStoreRetriever와 search_type, [8.2 키워드와 앙상블 검색](./02-키워드와-앙상블-검색.md)에서 배운 EnsembleRetriever, [8.3 멀티쿼리와 RAG Fusion](./03-멀티쿼리와-rag-fusion.md)에서 배운 검색 다양성 향상 기법
...
이번 섹션에서는 검색 결과를 **후처리**하여 품질을 높이는 방법을 배웠습니다. 다음 섹션 [8.5 셀프 쿼리와 메타데이터 필터링](./05-셀프-쿼리-검색기.md)에서는 검색의 **전처리** 단계로 돌아가, 사용자의 자연어 쿼리를 자동으로 분석하여 메타데이터 기반 필터 조건을 생성하는 **SelfQueryRetriever**를 학습합니다. 컨텍스트 압축이 "찾아온 것에서 골라내기"라면, 셀프 쿼리는 "처음부터 정확히 찾기"에 해당하는 전략이죠.
...
- [baseretriever](./01-검색기-기초.md) (prerequisite)
