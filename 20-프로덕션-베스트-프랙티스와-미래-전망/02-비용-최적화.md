...
**선수 지식**: [Session 20.1: 프로덕션 보안](./01-프로덕션-보안.md)에서 배운 보안 계층 구조, [Session 16.2: 커스텀 콜백 핸들러](16-콜백과-관찰-가능성/02-커스텀-콜백-핸들러.md)에서 다룬 `CostTrackingHandler`와 Callback 패턴, [Chapter 5: LCEL](05-lcellangchain-expression-language-마스터/01-lcel-기초와-파이프-연산자.md)에서 배운 `RunnableBranch`와 파이프 연산자
...
스마트 라우팅, 캐싱, 토큰 예산 관리를 조합하면 **비용을 40~76%까지 절감**하면서도 사용자 만족도 95% 이상을 유지할 수 있습니다. 앞서 [Session 20.1](./01-프로덕션-보안.md)에서 보안 계층을 쌓았으니, 이번에는 그 위에 **비용 방어막**을 구축해 봅시다.
...
[Session 16.2: 커스텀 콜백 핸들러](16-콜백과-관찰-가능성/02-커스텀-콜백-핸들러.md)에서 `CostTrackingHandler`를 만들어 호출별 비용을 계산하고 누적 추적하는 패턴을 배웠습니다. 여기서는 그 기반 위에 **프로덕션 수준의 예산 강제(Budget Enforcement)** 로직을 추가합니다. 핵심 차이는 두 가지입니다:
...
이 예산 관리자를 LangChain 콜백과 연결하려면, [Session 16.2](16-콜백과-관찰-가능성/02-커스텀-콜백-핸들러.md)에서 만든 `CostTrackingHandler`를 **확장**합니다. 기본 토큰 추출과 비용 계산 로직은 이미 `CostTrackingHandler`에 구현되어 있으므로, 여기서는 **예산 강제 로직만 추가**하면 됩니다.
...
프로덕션 환경에서는 [Session 16.2](16-콜백과-관찰-가능성/02-커스텀-콜백-핸들러.md)의 `CostTrackingHandler`가 제공하는 호출별 비용 기록(`cost_history`)과 리포트(`get_report()`) 위에, **구조화된 로깅과 외부 알림 연동**을 추가해야 합니다. 기본 비용 추적 로직을 다시 구현하는 대신, Ch16.2의 핸들러를 **상속하여 확장**합니다.
...
[Session 16.2](16-콜백과-관찰-가능성/02-커스텀-콜백-핸들러.md)에서 배운 `CostTrackingHandler`의 비용 기록 능력과, 이 섹션에서 새로 만든 `TokenBudget`의 예산 강제 능력을 조합하는 것이 핵심입니다.
...
> 🔥 **실무 팁**: 비용 최적화의 첫 번째 단계는 **측정**입니다. [Session 16.2](16-콜백과-관찰-가능성/02-커스텀-콜백-핸들러.md)의 `CostTrackingHandler`를 먼저 붙여서 최소 2주간 비용 데이터를 수집한 후에 최적화를 시작하세요. 어떤 체인이 비용을 많이 쓰는지, 캐시 히트율은 얼마인지, 어떤 시간대에 요청이 몰리는지 파악해야 올바른 최적화 전략을 세울 수 있습니다. LangSmith의 태그 기능(`tags=["feature:search", "user:premium"]`)을 사용하면 기능별·사용자 티어별 비용 분석이 바로 가능합니다.
...
비용을 최적화했으니, 이제 **성능과 확장성**을 챙길 차례입니다. 다음 섹션 [Session 20.3: 성능 최적화와 확장](20-프로덕션-베스트-프랙티스와-미래-전망/03-확장성과-성능.md)에서는 비동기 처리, 스트리밍, 수평 확장 전략 등 LangChain 애플리케이션이 수천 명의 동시 사용자를 감당하는 방법을 다룹니다. 비용과 성능은 종종 트레이드오프 관계이기 때문에, 이번 섹션에서 배운 비용 감각이 큰 도움이 될 거예요.
