...
> 📊 **그림 1**: PydanticOutputParser의 내부 동작 흐름

```mermaid
flowchart LR
    A["LLM 텍스트 출력"] --> B["JSON 추출<br/>JsonOutputParser"]
    B --> C{"JSON 파싱 성공?"}
    C -->|성공| D["model_validate()<br/>Pydantic 검증"]
    C -->|실패| E["OutputParserException"]
    D --> F{"검증 통과?"}
    F -->|통과| G["Pydantic 모델<br/>인스턴스 반환"]
    F -->|실패| E
```
...
classDiagram
    ...
    RestaurantAnalysis --> "*" Review : reviews
    Review --> "*" RatingDetail : ratings
```
...
> 📊 **그림 3**: Pydantic 검증 단계 — field_validator와 model_validator의 실행 순서

```mermaid
flowchart TD
    A["JSON 데이터 입력"] --> B["타입 변환<br/>str '8.5' -> float 8.5"]
    B --> C["Field 제약 조건<br/>ge, le, max_length"]
    C --> D["field_validator<br/>개별 필드 검증"]
    D --> E["모델 인스턴스 생성"]
    E --> F["model_validator<br/>필드 간 관계 검증"]
    F --> G["최종 검증 완료<br/>인스턴스 반환"]
    C -->|실패| H["ValidationError"]
    D -->|실패| H
    F -->|실패| H
```
...
> 📊 **그림 4**: LCEL 체인에서 PydanticOutputParser의 데이터 흐름
...
sequenceDiagram
    ...
    participant PA as PydanticOutputParser
    participant R as 결과
    U->>P: concept='Docker'
    Note over P: format_instructions<br/>JSON Schema 주입
    P->>L: 완성된 프롬프트 전송
    L->>PA: JSON 포함 텍스트 응답
    PA->>PA: JSON 추출 + model_validate()
    PA->>R: TechConcept 인스턴스 반환
```
