...
**선수 지식**: [5.2 핵심 Runnable 컴포넌트](./02-핵심-runnable-컴포넌트.md)에서 배운 `RunnableParallel` 기초, [5.1 LCEL 기초와 파이프 연산자](./01-lcel-기초와-파이프-연산자.md)에서 배운 `invoke`/`stream`/`batch` 인터페이스
...
[5.2 핵심 Runnable 컴포넌트](./02-핵심-runnable-컴포넌트.md)에서 `RunnableParallel`의 기본 사용법을 배웠는데요, 이번에는 **내부 동작 원리**와 **성능 최적화 관점**에서 깊이 들어가 보겠습니다.
...
이번 섹션에서 성능을 높이는 **병렬 실행**과 **비동기 처리**를 배웠다면, 다음 섹션 [5.5 스트리밍과 이벤트 처리](./05-체인-구성과-바인딩.md)에서는 `astream_events`와 `astream_log`를 활용하여 체인 실행 중간 과정을 **실시간으로 관찰**하고, 사용자에게 **점진적으로 결과를 보여주는** 스트리밍 패턴을 학습합니다. 특히 복잡한 파이프라인에서 각 단계의 중간 결과를 추적하는 고급 스트리밍 기법을 다룹니다.
