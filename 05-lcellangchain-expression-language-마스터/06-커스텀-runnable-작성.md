...
**선수 지식**: 앞서 [5.1: LCEL 기초와 파이프 연산자](./01-lcel-기초와-파이프-연산자.md)에서 배운 Runnable 프로토콜(invoke/stream/batch), [5.2: 핵심 Runnable 컴포넌트](./02-핵심-runnable-컴포넌트.md)에서 배운 RunnableLambda, [5.5: 체인 구성과 바인딩](./05-체인-구성과-바인딩.md)에서 배운 with_fallbacks/with_retry 등의 개념이 필요합니다.
...
[5.2: 핵심 Runnable 컴포넌트](./02-핵심-runnable-컴포넌트.md)에서 `RunnableLambda`로 일반 함수를 Runnable로 감쌀 수 있다는 걸 배웠죠? `@chain` 데코레이터는 그 과정을 더 우아하게 만들어줍니다. 핵심 차이는 **함수 내부에서 다른 Runnable을 호출하면 자동으로 트레이싱 의존성이 기록**된다는 점입니다.
...
[5.4: 병렬 실행과 성능 최적화](./04-병렬-실행과-성능-최적화.md)에서 `astream`의 중요성을 배웠는데요, `RunnableLambda`에는 치명적인 약점이 하나 있습니다. **입력을 모두 버퍼링한 후에야 처리를 시작**한다는 점이죠. LLM이 토큰을 하나씩 스트리밍하는데, 중간에 `RunnableLambda`가 끼어 있으면 스트리밍이 끊깁니다.
...
> 5. [5.5: 체인 구성과 바인딩](./05-체인-구성과-바인딩.md)에서 배운 `with_listeners()`로 중간 단계 로깅
...
다음 [Chapter 6: 문서 로더와 텍스트 분할](06-문서-로더와-텍스트-분할/01-문서-로더-기초.md)에서는 LCEL로 구축한 파이프라인에 **외부 데이터를 연결하는 방법**을 배웁니다. PDF, 웹페이지, 데이터베이스 등 다양한 소스에서 문서를 불러오고, RAG를 위해 적절한 크기로 분할하는 기법을 다룹니다. LCEL 체인의 진정한 위력은 외부 데이터와 결합할 때 발휘되거든요!
...
- [runnable_parallel](./02-핵심-runnable-컴포넌트.md) (prerequisite)
- [stream_method](./01-lcel-기초와-파이프-연산자.md) (prerequisite)
- [async_execution](./04-병렬-실행과-성능-최적화.md) (prerequisite)
- [with_fallbacks_method](./05-체인-구성과-바인딩.md) (prerequisite)
- [with_listeners_method](./05-체인-구성과-바인딩.md) (prerequisite)
