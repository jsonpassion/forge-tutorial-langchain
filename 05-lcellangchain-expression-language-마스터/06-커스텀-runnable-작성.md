...
> 📊 **그림 1**: 커스텀 Runnable 3가지 접근법 — 간편함 vs 자유도

```mermaid
flowchart LR
    A["커스텀 Runnable"] --> B["@chain 데코레이터<br/>간편함: 높음<br/>자유도: 중간"]
    A --> C["RunnableGenerator<br/>간편함: 중간<br/>자유도: 중간"]
    A --> D["Runnable 서브클래스<br/>간편함: 낮음<br/>자유도: 높음"]
    B --> E["내부에서 다른<br/>Runnable 호출 시"]
    C --> F["스트리밍 보존이<br/>필요할 때"]
    D --> G["스키마 제어 +<br/>완전한 커스터마이징"]
```
...
> 📊 **그림 2**: RunnableLambda vs RunnableGenerator — 스트리밍 보존 차이

```mermaid
flowchart TD
    subgraph BAD["RunnableLambda — 스트리밍 끊김"]
        direction LR
        A1["LLM"] -->|"토큰 1,2,3..."| B1["RunnableLambda<br/>전체 버퍼링"]
        B1 -->|"완성된 전체 텍스트"| C1["다음 단계"]
    end
    subgraph GOOD["RunnableGenerator — 스트리밍 보존"]
        direction LR
        A2["LLM"] -->|"토큰 1"| B2["RunnableGenerator<br/>즉시 변환"]
        B2 -->|"변환된 토큰 1"| C2["다음 단계"]
        A2 -->|"토큰 2"| B2
        B2 -->|"변환된 토큰 2"| C2
    end
```
...
> 📊 **그림 3**: Runnable 서브클래스 — 구현해야 할 메서드 구조
...
classDiagram
    class Runnable {
        ...
        +atransform(input, config) AsyncIterator
        +get_graph() Graph
        +input_schema PropertyModel
        +output_schema PropertyModel
    }
    ...
    Runnable <|-- TextStatistics : 필수: invoke만
    Runnable <|-- StreamingWordCounter : 권장: invoke + transform
```
...
> 📊 **그림 4**: 체인 디버깅 도구 — 4가지 검사 메서드

```mermaid
flowchart TD
    A["복잡한 LCEL 체인"] --> B["get_graph().print_ascii()<br/>구조 시각화"]
    A --> C["input_schema<br/>입력 타입 확인"]
    A --> D["output_schema<br/>출력 타입 확인"]
    A --> E["get_prompts()<br/>프롬프트 목록 조회"]
    B --> F["draw_mermaid()<br/>Mermaid 다이어그램 생성"]
    B --> G["draw_mermaid_png()<br/>PNG 이미지 생성"]
```
...
> 📊 **그림 5**: 감정 분석 파이프라인 — 실습 전체 흐름

```mermaid
sequenceDiagram
    participant U as 사용자 입력
    participant SC as @chain<br/>sentiment_pipeline
    participant P1 as 감정 분석 프롬프트
    participant LLM as ChatOpenAI
    participant P2 as 조언 프롬프트
    participant RG as RunnableGenerator<br/>emoji_tagger
