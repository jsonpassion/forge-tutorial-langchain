...
이 섹션에서는 OpenAI 외에 Anthropic(Claude), Google(Gemini), 그리고 Ollama를 통한 로컬 오픈소스 모델까지, 다양한 LLM 프로바이더를 LangChain에서 연동하는 방법을 다룹니다. 앞서 [Session 2.1: ChatOpenAI 심화](./01-chatopenai-심화.md)에서 배운 `temperature`, `max_tokens` 같은 파라미터 개념이 다른 프로바이더에서도 동일하게 적용되는 것을 확인하게 됩니다.
