...
이 섹션에서는 LangChain 프레임워크를 중심으로 형성된 **생태계 전체**를 조망합니다. 앞서 [Session 1.4: 첫 번째 LangChain 애플리케이션](./04-첫-번째-langchain-애플리케이션.md)에서 만든 체인을 **관찰하고(LangSmith)**, **배포하고(LangServe)**, **확장하는(LangGraph)** 도구들을 살펴보겠습니다.
...
LCEL 체인은 `A | B | C`로 직선적이지만, LangGraph는 이처럼 **분기, 합류, 반복**이 가능합니다. 이 능력이 진가를 발휘하는 건 [Ch13: LangGraph 기초](13-langgraph-기초/01-langgraph-소개와-핵심-개념.md)에서 본격적으로 다룹니다.
...
다음 챕터 [Ch2: LLM과 Chat Model 다루기](02-llm과-chat-model-다루기/01-chatopenai-심화.md)에서는 LangChain의 핵심인 **모델 계층**을 본격적으로 파고듭니다. ChatOpenAI를 넘어 다양한 모델 프로바이더를 다루고, `temperature`, `max_tokens`, `top_p` 같은 파라미터가 모델 출력에 어떤 영향을 미치는지 실험합니다. 또한 모델을 교체해도 코드를 수정할 필요가 없는 LangChain의 **통합 인터페이스**가 빛을 발하는 순간을 직접 체험하게 될 것입니다.
