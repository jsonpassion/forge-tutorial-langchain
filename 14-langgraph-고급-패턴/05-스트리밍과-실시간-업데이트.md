...
![SSE 프로토콜 시퀀스 다이어그램 — TCP 3-way 핸드셰이크 후 클라이언트가 Accept: text/event-stream 헤더로 요청하면, 서버가 Content-Type: text/event-stream과 Transfer-Encoding: chunked로 연결을 유지하며 반복적으로 data를 전송하는 흐름](../images/ch14/wfv3zbdxefcsf69ym2l4-af09b0e0.png "Server-Sent Events Explained — DEV Community")
...
![WebSocket 연결(양방향 지속 연결)과 HTTP 연결(요청-응답 후 종료) 비교 다이어그램 — SSE는 HTTP 기반 단방향 스트리밍으로 WebSocket보다 단순하면서 LLM 토큰 전달에 최적화됨](../images/ch14/24538662-d00f-4457-a8a4-9da16f618046-d7698a9e.png "freeCodeCamp — Server-Sent Events vs WebSockets")
