...
> 📊 **그림 1**: LangGraph 에이전트 구축 5단계 패턴

```mermaid
flowchart LR
    A["1. 상태 정의<br/>TypedDict"] --> B["2. 노드 작성<br/>Python 함수"]
    B --> C["3. 엣지 연결<br/>add_edge"]
    C --> D["4. 컴파일<br/>compile()"]
    D --> E["5. 실행<br/>invoke / stream"]
```
...
> 📊 **그림 2**: 상태 스키마 — 노드 간 공유 상태 구조

```mermaid
flowchart TD
    S["AgentState"] --> Q["query: str<br/>덮어쓰기"]
    S --> R["response: str<br/>덮어쓰기"]
    S --> ST["steps: list<br/>Annotated - 누적"]
    N1["노드 A"] -->|읽기/쓰기| S
    N2["노드 B"] -->|읽기/쓰기| S
```
...
> 📊 **그림 3**: 덮어쓰기 vs 리듀서 누적 — 상태 업데이트 비교

```mermaid
flowchart TD
    subgraph 덮어쓰기["일반 필드 - 덮어쓰기"]
        A1["노드 A: response = 'X'"] --> A2["노드 B: response = 'Y'"]
        A2 --> A3["최종: response = 'Y'"]
    end
    subgraph 누적["Annotated 필드 - 리듀서 누적"]
        B1["노드 A: steps = ('분석')"] --> B2["노드 B: steps = ('생성')"]
        B2 --> B3["최종: steps = ('분석', '생성')"]
    end
```
...
sequenceDiagram
    ...
    participant N1 as analyze_query
    participant N2 as generate_response
    S->>N1: 전체 상태 전달
    N1->>S: steps만 반환 (누적)
    S->>N2: 전체 상태 전달
    N2->>S: response + steps 반환
```
...
> 📊 **그림 5**: 실습 에이전트 — 3단계 QA 파이프라인 구조

```mermaid
flowchart LR
    ST(("START")) --> C["classify<br/>질문 분류"]
    C --> L["llm<br/>LLM 호출"]
    L --> F["format<br/>응답 포맷팅"]
    F --> EN(("END"))
    C -.->|category 설정| L
    L -.->|answer 설정| F
```
