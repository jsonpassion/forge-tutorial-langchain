...
이 섹션에서는 LangGraph의 프리빌트(prebuilt) 컴포넌트인 `ToolNode`와 `tools_condition`을 사용하여, LLM이 도구를 호출하고 그 결과를 관찰하며 다시 추론하는 **도구 호출 루프**를 구현합니다. [세션 13.3: 조건부 엣지와 라우팅](./03-조건부-엣지와-라우팅.md)에서 배운 `add_conditional_edges`와 `should_continue` 패턴이 실제 도구 호출 에이전트에서 어떻게 쓰이는지 확인하게 됩니다.

**선수 지식**: 
- [세션 13.1](./01-langgraph-소개와-핵심-개념.md)의 StateGraph, 노드, 엣지 개념
- [세션 13.2](./02-첫-번째-langgraph-에이전트.md)의 그래프 구성과 invoke/stream 실행
- [세션 13.3](./03-조건부-엣지와-라우팅.md)의 `add_conditional_edges`와 라우팅 함수
- [챕터 11](11-도구tools와-함수-호출/01-도구-정의와-바인딩.md)의 도구(Tools)와 함수 호출 기본 개념
...
[세션 13.3](./03-조건부-엣지와-라우팅.md)에서 `should_continue` 함수를 직접 작성하여 에이전트 루프를 만들었던 것, 기억나시죠? 매번 "마지막 메시지에 tool_calls가 있으면 도구 노드로, 없으면 END로"라는 동일한 패턴을 반복 작성하는 건 비효율적입니다.
...
앞서 [세션 13.3](./03-조건부-엣지와-라우팅.md)에서 도구를 실행하려면 노드 함수 안에서 직접 도구를 찾고 호출하는 코드를 작성해야 했습니다. `ToolNode`는 이 과정 전체를 캡슐화합니다.
...
[세션 13.3](./03-조건부-엣지와-라우팅.md)에서 작성했던 `should_continue` 함수를 떠올려 보세요:
...
- [should_continue](./03-조건부-엣지와-라우팅.md) (prerequisite)
