...
> 📊 **그림 1**: 프로덕션 RAG의 핵심 과제와 해결 전략

```mermaid
flowchart TD
    subgraph 과제["프로덕션 RAG 3대 과제"]
        Q1["문서 동기화<br/>매번 전체 재인덱싱?"]
        Q2["품질 추적<br/>어디서 병목이 생겼지?"]
        Q3["확장성<br/>동시 사용자 100명이면?"]
    end
    subgraph 해결["해결 전략"]
        S1["RecordManager<br/>+ Indexing API"]
        S2["LangSmith<br/>+ 커스텀 콜백"]
        S3["다계층 캐싱<br/>+ 비동기 처리"]
    end
    Q1 --> S1
    Q2 --> S2
...
> 📊 **그림 2**: RecordManager 기반 인덱싱 흐름 — 문서 변경 감지와 처리

```mermaid
flowchart TD
    A["문서 로드"] --> B["콘텐츠 해시 계산"]
    B --> C{"RecordManager에<br/>해시 존재?"}
    C -->|"없음 - 신규"| D["벡터 스토어에 추가"]
    C -->|"있음"| E{"해시 동일?"}
    E -->|"동일 - 변경 없음"| F["건너뜀<br/>num_skipped +1"]
    E -->|"다름 - 수정됨"| G["이전 버전 삭제<br/>+ 새 버전 추가"]
    D --> H["RecordManager에<br/>해시/시간/소스ID 기록"]
    G --> H
    H --> I["인덱싱 결과 반환"]
```
...
> 📊 **그림 3**: incremental vs full cleanup 모드 — 삭제 처리 차이

```mermaid
flowchart LR
    subgraph INC["incremental 모드"]
        direction TB
        I1["로더가 반환한<br/>소스 ID만 검사"] --> I2{"변경/삭제?"}
        I2 -->|"변경"| I3["업데이트"]
        I2 -->|"삭제"| I4["벡터에서 제거"]
        I2 -->|"로더 미반환"| I5["유령 문서로 잔존"]
    end
    subgraph FULL["full 모드"]
        direction TB
        F1["이번 실행의<br/>전체 문서 목록 확인"] --> F2{"벡터 스토어에<br/>있지만 목록에 없음?"}
        F2 -->|"있음"| F3["무조건 삭제"]
        F2 -->|"없음"| F4["유지"]
    end
```
...
> 📊 **그림 4**: 프로덕션 임베딩 캐시 — Redis 네임스페이스 계층 구조

```mermaid
graph TD
    R["Redis 캐시"] --> M1["emb:v1:text-embedding-3-small"]
    R --> M2["emb:v1:text-embedding-3-large"]
    M1 --> T1["tenant:company_a"]
    M1 --> T2["tenant:company_b"]
    M2 --> T3["tenant:company_a"]
    R --> E1["emb:staging:text-embedding-3-small"]
    R --> E2["emb:production:text-embedding-3-small"]
...
> 📊 **그림 5**: 인덱싱 파이프라인과 쿼리 파이프라인 분리 아키텍처

```mermaid
flowchart TD
    subgraph IDX["인덱싱 파이프라인 - 비동기/배치"]
        direction LR
        DS["문서 소스"] --> LDR["로더"]
        LDR --> SPL["분할기"]
        SPL --> EMB1["임베딩"]
        EMB1 --> VS1["벡터 스토어"]
        SCH["스케줄러<br/>Cron/Webhook"] -.-> DS
        RM["RecordManager<br/>변경 추적"] -.-> VS1
    end
    subgraph QRY["쿼리 파이프라인 - 실시간"]
        direction LR
        UQ["사용자 질문"] --> CC{"캐시 확인"}
        CC -->|"히트"| ANS["즉시 응답"]
        CC -->|"미스"| EMB2["임베딩"]
        EMB2 --> VS2["벡터 검색"]
        VS2 --> LLM["LLM 생성<br/>스트리밍"]
        LLM --> ANS
    end
    VS1 -.->|"공유"| VS2
```
