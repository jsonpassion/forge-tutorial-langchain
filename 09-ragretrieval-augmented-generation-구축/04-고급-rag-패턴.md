...
이 섹션에서는 기본 RAG 파이프라인의 검색 품질을 극적으로 향상시키는 **4가지 고급 패턴**을 학습합니다. [기본 RAG 체인 구축](ch09/session_01.md)에서 만든 단순 벡터 검색을 넘어, 검색 정밀도와 문맥 풍부함을 동시에 잡는 실전 기법들을 다룹니다.
...
> 📊 **그림 1**: 기본 RAG의 3가지 한계와 이를 해결하는 고급 패턴

```mermaid
flowchart TD
    subgraph PROBLEM["Naive RAG의 한계"]
        P1["청크 크기 딜레마<br/>작으면 정밀, 크면 문맥"]
        P2["쿼리-문서 의미 격차<br/>짧은 질문 vs 긴 문서"]
        P3["단일 쿼리 한계<br/>복잡한 질문 커버 불가"]
    end
    subgraph SOLUTION["고급 RAG 패턴"]
        S1["Parent Document Retriever"]
        S2["HyDE"]
        S3["재랭킹(Cross-Encoder)"]
        S4["쿼리 분해 / Multi-Query"]
    end
    P1 --> S1
    P2 --> S2
    P1 --> S3
...
> 📊 **그림 2**: Parent Document Retriever의 인덱싱과 검색 흐름

```mermaid
flowchart LR
    subgraph INDEX["인덱싱 단계"]
        D["원본 문서"] --> P["부모 청크<br/>2000자"]
        P --> C["자식 청크<br/>400자"]
        C --> V["벡터 스토어<br/>자식 임베딩 저장"]
        P --> DS["문서 저장소<br/>부모 원문 보관"]
    end
    subgraph SEARCH["검색 단계"]
        Q["사용자 쿼리"] --> V2["벡터 검색<br/>자식 매칭"]
        V2 --> M["매칭된 자식"]
        M --> R["부모 청크 반환<br/>풍부한 문맥"]
    end
    V --> V2
    DS --> R
```
...
![HyDE 모델 구조 — instruction+query → GPT가 가상 문서 생성 → Contriever 임베딩 → 실제 문서 검색 (한국어 예시 포함)](../images/ch09/Fig_2_Illustration_of_Hy_DE_model_258ef0-aef94cf1.png "Zilliz — Improve RAG and Information Retrieval with HyDE")


> 📊 **그림 3**: HyDE의 동작 흐름 — 가상 문서로 의미 격차 해소
...
![Retrieve & Re-Rank 파이프라인 — Bi-Encoder로 후보 검색 후 Cross-Encoder로 재랭킹](../images/ch09/InformationRetrieval-5323cbe3.png "Sentence-Transformers (SBERT) 공식 문서")


> 📊 **그림 4**: 2단계 검색 — bi-encoder로 후보 선별, cross-encoder로 재랭킹

```mermaid
flowchart LR
    Q["쿼리"] --> BE["1단계: Bi-Encoder<br/>독립 임베딩"]
    BE --> C20["상위 20개 후보<br/>빠른 검색"]
    C20 --> CE["2단계: Cross-Encoder<br/>쿼리+문서 함께 평가"]
    CE --> TOP["상위 3~5개<br/>정밀 선별"]
    TOP --> LLM["LLM 생성"]
...
> 📊 **그림 5**: 쿼리 분해 두 가지 전략 비교 — Multi-Query vs Query Decomposition

```mermaid
flowchart TD
    Q["복잡한 질문"] --> MQ["Multi-Query<br/>같은 질문, 다른 표현"]
    Q --> QD["Query Decomposition<br/>서브 질문으로 분해"]
    MQ --> V1["변형 쿼리 1"]
    MQ --> V2["변형 쿼리 2"]
    MQ --> V3["변형 쿼리 3"]
    QD --> S1["서브 질문 A"]
