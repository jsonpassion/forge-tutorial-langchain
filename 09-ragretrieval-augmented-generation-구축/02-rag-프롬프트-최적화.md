...
이 섹션에서는 RAG 파이프라인의 "생성(Generation)" 단계에 집중합니다. 앞서 [9.1: 기본 RAG 체인 구축](./01-기본-rag-체인-구축.md)에서 검색된 문서를 LLM에 전달하는 기본 구조를 만들었다면, 이번에는 **어떻게 전달하느냐**에 따라 답변 품질이 극적으로 달라지는 것을 체험합니다.
