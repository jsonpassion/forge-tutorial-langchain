...
> 📊 **그림 1**: RAG 시스템 프롬프트의 4가지 핵심 요소

```mermaid
graph TD
    P["RAG 시스템 프롬프트"] --> A["1. 역할 정의<br/>LLM의 정체성 규정"]
    P --> B["2. 컨텍스트 사용 규칙<br/>제공된 문서만 사용"]
    P --> C["3. 답변 불가 처리<br/>모를 때의 행동 지침"]
    P --> D["4. 출력 형식 지정<br/>답변 구조와 톤"]
...
> 📊 **그림 2**: 컨텍스트 포맷팅 3가지 전략 비교

```mermaid
flowchart LR
    A["검색된 문서"] --> B["전략 1<br/>메타데이터 포함"]
    A --> C["전략 2<br/>XML 태그 기반"]
    A --> D["전략 3<br/>document_prompt"]
    B --> E["(문서 N) 출처: file<br/>내용..."]
    C --> F["'document' 태그로<br/>경계 명확화"]
    D --> G["LangChain 내장<br/>자동 포맷팅"]
...
> 📊 **그림 3**: 소스 인용 구현 3가지 접근법 — 복잡도와 정확도의 트레이드오프

```mermaid
flowchart TD
    subgraph M1["방법 1: 프롬프트 기반"]
        A1["프롬프트에<br/>인용 형식 지시"] --> B1["텍스트 인용<br/>예: 내용(1)(3)"]
    end
    subgraph M2["방법 2: 구조화된 출력"]
        A2["Pydantic 모델<br/>Citation 스키마"] --> B2["프로그래밍적<br/>인용 데이터"]
    end
    subgraph M3["방법 3: LCEL 체인"]
        A3["Retriever + Prompt<br/>+ Structured LLM"] --> B3["완전 자동화<br/>인용 파이프라인"]
    end
    M1 -.->|"복잡도 증가"| M2
    M2 -.->|"복잡도 증가"| M3
```
...
stateDiagram-v2
    ...
    질문수신 --> 문서검색
    문서검색 --> LLM판단
    LLM판단 --> ANSWERED: 충분한 정보 존재
    LLM판단 --> PARTIAL: 일부 정보만 존재
    LLM판단 --> NOT_FOUND: 관련 정보 없음
    ANSWERED --> 답변출력: 답변 + 출처 표시
    PARTIAL --> 부분답변: 가능한 답변 + 부족한 부분 명시
    NOT_FOUND --> 안내응답: 답변 불가 안내 + 질문 제안
    답변출력 --> [*]
    부분답변 --> [*]
    안내응답 --> [*]
```
...
> 📊 **그림 5**: 프로덕션 RAG 체인 전체 아키텍처

```mermaid
flowchart LR
    Q["사용자 질문"] --> RP["RunnableParallel"]
    RP --> R["Retriever<br/>문서 검색"]
    RP --> PT["질문 통과<br/>PassThrough"]
    R --> FM["format_docs<br/>메타데이터 포맷팅"]
    FM --> PR["프로덕션<br/>RAG 프롬프트"]
    PT --> PR
    PR --> SL["Structured LLM<br/>RAGAnswer 스키마"]
    SL --> HC["handle_response<br/>상태별 후처리"]
    HC --> OUT["최종 응답<br/>인용 + 상태 포함"]
...
![RAG 원본 논문 아키텍처 — Query Encoder + Document Index + Generator](../images/ch09/x1-e6c49d49.png "Lewis et al., Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks (2020)")
...
![Lost in the Middle — 문서 위치에 따른 LLM 성능 변화 (U자형 곡선)](../images/ch09/x1-3f42a9f9.png "Liu et al., Lost in the Middle: How Language Models Use Long Contexts (2023)")
