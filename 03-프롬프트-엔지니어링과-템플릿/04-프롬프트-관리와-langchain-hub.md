...
**선수 지식**: [세션 3.1: ChatPromptTemplate 기초](session_3_1.md)에서 배운 `ChatPromptTemplate` 생성과 `invoke()`, [세션 3.3: Few-shot 프롬프팅](session_3_3.md)에서 배운 `FewShotChatMessagePromptTemplate`
...
> 📊 **그림 1**: LangChain Hub 기반 프롬프트 관리 전체 흐름

```mermaid
flowchart LR
    A["커뮤니티 프롬프트"] -->|hub.pull| B["로컬 프로젝트"]
    B -->|수정/개선| C["커스텀 프롬프트"]
    C -->|hub.push| D["LangChain Hub"]
    D -->|버전 관리| E["커밋 + 태그"]
    E -->|hub.pull :prod| F["프로덕션 배포"]
```
...
> 📊 **그림 2**: 프롬프트 버전 관리 — 커밋과 태그의 관계

```mermaid
flowchart TD
    P1["hub.push v1"] --> C1["커밋 a3f2b1"] 
    P2["hub.push v2"] --> C2["커밋 c9839f"]
    P3["hub.push v3"] --> C3["커밋 e7d4a2"]
    C1 -.->|태그| T1["dev"]
    C2 -.->|태그| T2["staging"]
    C3 -.->|태그| T3["prod"]
    T3 -->|"hub.pull :prod"| APP["프로덕션 앱"]
```
...
> 📊 **그림 3**: 태그 기반 배포 — 프롬프트 승격 흐름

```mermaid
stateDiagram-v2
    [*] --> Dev: hub.push + tag 'dev'
    Dev --> Staging: 검증 완료
    Staging --> Prod: QA 통과
    Prod --> [*]: 서비스 반영
    note right of Dev: 개발/실험 단계
    note right of Staging: 팀 리뷰 단계
    note right of Prod: 코드 변경 없이<br/>태그만 이동
```

### 개념 4: 프롬프트 평가 기초

> 📊 **그림 4**: 데이터셋 기반 프롬프트 평가 파이프라인

```mermaid
flowchart TD
    DS["1. 데이터셋<br/>입력 + 기대 출력"] --> TF["2. 타겟 함수<br/>프롬프트 + LLM"]
    TF --> EV["3. 평가자<br/>점수 매기기"]
    EV --> R["평가 결과"]
    R -->|개선 필요| P["프롬프트 수정"]
    P -->|새 버전| TF
    R -->|품질 충분| D["프로덕션 배포"]
...
> 💡 **알고 계셨나요?**: `hub.pull()`로 가져온 객체는 일반 `ChatPromptTemplate`과 동일합니다. 즉, [세션 3.2](session_3_2.md)에서 배운 프롬프트 합성(`+` 연산자)이나 `MessagesPlaceholder`를 그대로 적용할 수 있습니다. Hub 프롬프트를 기반으로 자신만의 확장 프롬프트를 만드는 것이 가장 효율적인 활용법이죠.
...
- [prompt_composition](../03-프롬프트-엔지니어링과-템플릿/02-고급-프롬프트-패턴.md) (prerequisite)
